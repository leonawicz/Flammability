...
files5 <- unlist(lapply(dirs3, function(x, year) list.files(x, pattern=paste0("_", year, "\\.tif$"), full=TRUE), year=yr))
s <- stack(c(files3, files5))
names(s) <- paste0(rep(c("GBM3", "GBM5"), each=length(dirs3)), paste0("_Flammability", c("", "_Lightning0.5", "0.5_Lightning 0.5")))
s <- brick(s)
...
dirs3 <- dirs[4]
dirs5 <- dirs[8]
files3 <- unlist(lapply(dirs3, function(x, year) list.files(x, pattern=paste0("\\.tif$"), full=TRUE)))
files5 <- unlist(lapply(dirs5, function(x, year) list.files(x, pattern=paste0("\\.tif$"), full=TRUE)))
...
# Write PNGs
png(paste0(plotDir, "/gbm.flamm_", yr,"_comparisons.png"), height=1600, width=2400, res=200)
...
p <- levelplot(s2, maxpixels=ncell(s)/10, main=paste("1977 3- vs. 5-GBM flammability maps"), par.settings=revRasterTheme, contour=T, margin=F)#, at=at.vals, colorkey=colkey)
print(p)
dev.off()

png(paste0(plotDir, "/flam_spacetime_threshDist_", threshold, ".png"), height=2400, width=2400, res=200)
p <- levelplot(s, maxpixels=ncell(s)/10, main=paste("1950-2009 0.175 flammability threshold distributions"), par.settings=revRasterTheme, contour=T, margin=F)#, at=at.vals, colorkey=colkey)
grid.newpage()
