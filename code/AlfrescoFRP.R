...
result.name <- paste0(outDir,"/firescarbrick_annual_observed_",gsub("_","",alf.domain), "_", tolower(emp.fire.cause), "_1940_2013.tif")
result2.name <- paste0(outDir,"/firescarlayer_total_observed_",gsub("_","",alf.domain), "_", tolower(emp.fire.cause), "_1940_2013.tif")
...
system(paste("ssh eris.snap.uaf.edu cp -r", file.path(templateDir, "*"), paste0(appDir, "/"))) # Copy template app files to new directory
