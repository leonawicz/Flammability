...
d2 %>% filter(Year >= 2014) %>% group_by(Model) %>%
...
(g5 <- ggplot(data=d.all, aes(x=Year, y=LightPred, label=Rank)) +
    ...
    labs(x="Year", y="Predicted number of strikes", title="1950-2011 CRU32 and 2014-2099 GCM GBM-predicted summer lightning strikes: time series") +
    facet_wrap(~ Model, ncol=2))
...
(g6 <- ggplot(data=d.all, aes(x=LightPred, label=Year)) +
    ...
    labs(x="Predicted number of strikes", y="CDF", title="1950-2011 CRU32 and 2014-2099 GCM GBM-predicted summer lightning strikes: empirical CDF") +
    facet_wrap(~ Model, ncol=2))
