...
CABvsTimePlot <- function(years, baseline.year){
	...
	png(file.path(outDir,paste("CAB_",years[1],"to",years[length(years)],".png",sep="")),res=120,width=1000,height=800)
	...
	plot(0,type="n",xlim=range(years),ylim=ylm,ylab=expression(paste(plain("Cumalitive Area Burn   "),("km"^2),sep="")),xlab='Year',
		main=paste("CAB ",years[1],"-",years[length(years)],": Statewide",sep=""),cex.axis=1.1,cex.lab=1.2)
	for (j in 1:numrep) lines(years,cab.alf.all[,j],lwd=1,lty=2,col="dark gray")
	lines(years,cab.emp.all,lwd=4,col=1)
	legend("topleft",c("Historical","ALFRESCO Rep"),col=c(1,"dark gray"),lty=c(1,2),bty="n",cex=1.2,lwd=c(4,1))
	dev.off()
}
