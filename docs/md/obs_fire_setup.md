...
if (all(file.exists(c(result.name, result2.name)))) {
    ...
    dummy <- capture.output(b.fid <- fireScarsFunVec(x = fah, y = r, year = yrs.all, 
        years.avail = yrs, field = "FIREID"))
    b.fid <- brick(b.fid)
    ...
    dummy <- capture.output(writeRaster(b.fid, b.fid.name, datatype = "FLT4S", 
        overwrite = T))
