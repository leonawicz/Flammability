...
function(){
	tabPanel("About",
		HTML(
		'<p style="text-align:justify">This R Shiny web application displays relative area burned time series and fire rotation period by buffer size around various locations.
		The modeled outputs come from ALFRESCO replicates and are compared with observational fire scar data.</p>'),
		br(),
		...
		div(class="row-fluid",
			...
			div(class="span4",
				...
				p(HTML('<ul>'),
					...
					HTML('</ul>'),
					HTML('<li>Source code available from author.</li>'),
				HTML('</ul>'))
			)
		),
		value="about"
