...
dir.create(outDir <- file.path("../data/gbmFlammability", period, model, out), 
    recursive = T, showWarnings = F)
dir.create(plotDir <- file.path("../plots/gbmFlammability", period, model, plot.out), 
    recursive = T, showWarnings = F)
```
...
f <- function(p, bins = 1) {
    ...
    if (any(grep("Summer", d.names))) {
        obj.names.list <- list(ls(pattern = "^m.*.T$", envir = .GlobalEnv), 
            ls(pattern = "^m.*.P$", envir = .GlobalEnv))
        for (i in 1:nrow(summary(get(gbm.names[p])))) {
            obj.names <- obj.names.list[[i]]
            n <- length(obj.names)
            tmp2 <- 0
            for (j in 1:n) tmp2 <- tmp2 + get(obj.names[j])
            if (length(grep("T$", obj.names.list[[i]]))) 
                tmp2 <- tmp2/n
        ...
        for (i in 1:nrow(summary(get(gbm.names[p])))) tmp <- cbind(tmp, as.numeric(get(paste0("m.", 
            d.names[i]))[get(ind.names[p]), ]))
    }
...
```r
getGBMpreds <- function(a, b, nam1, nam2) predict.gbm(get(nam1[b]), get(nam2[a]), 
    n.trees = tree.numbers[b])
```
